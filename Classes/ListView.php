<?php
namespace mehrwert\MwKeywordlist;
...
class ListView extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	 ...
	 * @todo	\TYPO3\CMS\Core\Utility\GeneralUtility::int_from_ver is deprecated in TYPO3 v4.6 and must be replaced
	...
	private function setContentPageTypes() {
		if (intval($this->conf['respectContentDoktypes'])) {
			if (!empty($this->conf['setCustomContentDoktypes'])) {
				$this->contentDoktypes = implode(',', \TYPO3\CMS\Core\Utility\GeneralUtility::intExplode(',', $this->conf['setCustomContentDoktypes']));
			} else {
